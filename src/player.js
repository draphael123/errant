...
export class Player {
  constructor(scene, phys, fx, sfx, hud) {
    ...
    this.blockT = 0; this.rollDir = new THREE.Vector3(0, 0, 1); this.rollT = 0; this.stunT = 0; this.hurtT = 0; this.landT = 0; this.stepSide = 0;
  ...
  respawn(pos, yaw = 0, fullHeal = true) {
    ...
    this.body.groundBox = null; this.body.onGround = false; this.canDouble = true; this.lastGround.copy(pos);
  }
  ...
  takeDamage(dmg, from, opts = {}) {
    ...
    if (this.state === 'block' && facingIt && !opts.unblockable) {
      if (this.blockT < 0.17 && opts.parryable !== false) { this.sfx('parry'); this.fx.burst(this.pos.clone().add(new THREE.Vector3(fwd.x * 0.8, 1.2, fwd.z * 0.8)), 0xfff2b0, 22, 7); this.fx.hitstop(0.09); this.fx.shake(0.35); this.hud.toast('PARRY', 0.9); return 'parried'; }
      ...
      this.vel.x += _w.x * 3.5; this.vel.z += _w.z * 3.5;
  ...
  heal(n) { this.hp = Math.min(this.hpMax, this.hp + n); this.hud.setHearts(this.hp, this.hpMax); }
  ...
  update(dt, input, camFwd, game) {
    ...
    if (ground) { this.coyote = PH.coyote; this.canDouble = true; this.airAttackUsed = false; this.lastGround.copy(this.pos); } else this.coyote -= dt;
    if (input.just('jump')) this.jumpBuf = PH.buffer; else this.jumpBuf -= dt;

    // wish direction (camera relative)
    const ax = input.axis();
    const rx = -camFwd.z, rz = camFwd.x; // camera right
    ...
    if (st === 'dead') { this.deadT += dt; applyMove = false; moveScale = 0; }
    else if (st === 'hurt') { this.hurtT -= dt; moveScale = 0; canTurn = false; if (this.hurtT <= 0) this.state = ground ? 'idle' : 'air'; }
    else if (st === 'stagger') { this.stunT -= dt; moveScale = 0; canTurn = false; if (this.stunT <= 0) this.state = 'idle'; }
    else if (st === 'roll') {
      ...
      // face the nearest threat while guarding
    ...
    if (applyMove) {
      ...
      if (this.alive && this.jumpBuf > 0 && this.state !== 'roll' && this.state !== 'hurt' && this.state !== 'stagger' && !(this.attack && !this.attack.move.air)) {
        if (this.coyote > 0) { v.y = PH.jump; this.coyote = 0; this.jumpBuf = 0; this.sfx('jump'); this.state = 'air'; this.fx.burst(this.pos.clone(), 0xcfe6c0, 6, 2, { flat: true, up: 1, gravity: 8, life: 0.3 }); }
        else if (this.canDouble && !ground) { v.y = PH.djump; this.canDouble = false; this.jumpBuf = 0; this.sfx('djump'); this.fx.ring(this.pos.clone(), 0x9fd3ff, 0.3, 1.6, 0.35, 0.08); this.fx.burst(this.pos.clone(), 0x9fd3ff, 10, 3, { flat: true, up: 0.5, gravity: 4, life: 0.35 }); }
      }
    ...
    const fallSpeed = -v.y;
    this.phys.step(b, dt);
    if (b.onGround && !this.wasGround) { // landed
      const hard = fallSpeed > 16; this.sfx('land', { hard }); this.landT = hard ? 0.22 : 0.1;
    ...
    if (ground && hs > 1 && this.state !== 'roll') { const prev = this.runPhase; this.runPhase += dt * (6 + hs * 1.3); if (Math.floor(prev / Math.PI) !== Math.floor(this.runPhase / Math.PI)) this.sfx('step'); }
  ...
  startRoll() {
    this.stamina -= STA.roll; this.staDelay = STA.delay; this.state = 'roll'; this.rollT = 0; this.attack = null; this.t = 0;
  ...
  startAttack(name, game) {
    ...
    // soft lock: face the nearest enemy in a forward cone, else face the stick
  ...
  hitTest(mv, game) {
    ...
    for (const e of game.enemies) {
      ...
      const res = e.takeHit({ dmg: mv.dmg * diff().playerDmg, poise: mv.poise, from: this.pos, knock: mv.knock, heavy: mv.name === 'heavy' });
      if (res) {
  ...
  pose(dt, ground, input) {
    ...
    let rate = 14;
    const idle = () => { q.bodyY = Math.sin(t * 2) * 0.012; q.armR = { x: 0.12, y: 0, z: 0.14 }; q.armL = { x: 0.08, y: 0, z: -0.16 }; q.foreR = -0.25; q.foreL = -0.35; q.foreLy = 0.3; q.headX = Math.sin(t * 0.7) * 0.04; q.headY = Math.sin(t * 0.5) * 0.08; q.cape = 0.05; q.hipR = 0; q.hipL = 0; q.kneeR = 0; q.kneeL = 0; q.lean = 0; q.yaw = 0; q.tumble = 0; q.roll = 0; };
    if (st === 'idle' || st === 'attack' && !this.attack) idle();
    else if (st === 'run') {
      const ph = this.runPhase, f = this.speedFrac;
      q.hipR = Math.sin(ph) * 0.9 * f; q.hipL = -Math.sin(ph) * 0.9 * f;
      q.kneeR = Math.max(0, Math.sin(ph - 1.1)) * 1.3 * f + 0.15; q.kneeL = Math.max(0, -Math.sin(ph - 1.1)) * 1.3 * f + 0.15;
      q.armR = { x: -Math.sin(ph) * 0.55 * f + 0.15, y: 0, z: 0.25 }; q.armL = { x: Math.sin(ph) * 0.6 * f, y: 0, z: -0.3 };
      q.foreR = -0.5; q.foreL = -0.7; q.foreLy = 0.3;
      q.lean = 0.2 * f; q.bodyY = Math.abs(Math.sin(ph)) * 0.05 * f - 0.03 * f; q.yaw = Math.sin(ph) * 0.08 * f;
      q.cape = 0.35 + 0.35 * f + Math.sin(ph * 2) * 0.08; q.headX = -0.1 * f; q.tumble = 0; q.roll = 0; rate = 16;
    }
    else if (st === 'air' && !this.attack) {
      const vy = this.vel.y; const rising = vy > 1;
      if (rising) { q.hipR = -0.7; q.hipL = 0.35; q.kneeR = 1.0; q.kneeL = 0.35; q.armR = { x: -0.7, y: 0, z: 0.6 }; q.armL = { x: -0.5, y: 0, z: -0.6 }; q.lean = 0.12; q.cape = 0.05; }
      else { q.hipR = 0.15; q.hipL = -0.15; q.kneeR = 0.5; q.kneeL = 0.7; q.armR = { x: -0.4, y: 0, z: 1.1 }; q.armL = { x: -0.3, y: 0, z: -1.1 }; q.lean = clamp(-vy * 0.012, -0.1, 0.35); q.cape = clamp(0.8 - vy * 0.05, 0.4, 1.6); }
      q.foreR = -0.4; q.foreL = -0.5; q.foreLy = 0.3; q.bodyY = 0; q.headX = rising ? -0.15 : 0.25; q.tumble = 0; q.roll = 0; rate = 10;
    }
      ...
      q.lean = 0.1; q.bodyY = -0.09; q.hipR = -0.35; q.hipL = -0.35; q.kneeR = 0.6; q.kneeL = 0.6; q.headX = 0.1; q.cape = 0.1; q.tumble = 0; q.roll = 0; q.yaw = -0.25; rate = 22;
    }
    else if (st === 'roll') {
      const u = clamp(this.rollT / PH.rollTime, 0, 1);
      q.tumble = u * Math.PI * 2; q.bodyY = -0.42 + Math.sin(u * Math.PI) * 0.1; q.hipR = -1.3; q.hipL = -1.3; q.kneeR = 1.9; q.kneeL = 1.9;
      q.armR = { x: -1.1, y: 0, z: 0.4 }; q.armL = { x: -1.1, y: 0, z: -0.4 }; q.foreR = -1.6; q.foreL = -1.6; q.foreLy = 0.3; q.headX = 0.6; q.lean = 0; q.cape = -0.4; q.roll = 0; rate = Infinity;
    }
    else if (st === 'hurt') { q.lean = -0.35; q.armR = { x: -0.7, y: 0, z: 0.9 }; q.armL = { x: -0.6, y: 0, z: -0.9 }; q.foreR = -0.5; q.foreL = -0.5; q.foreLy = 0.3; q.headX = -0.45; q.bodyY = 0.02; q.hipR = -0.3; q.hipL = 0.2; q.kneeR = 0.5; q.kneeL = 0.3; q.cape = -0.3; q.tumble = 0; q.roll = 0; rate = 30; }
    else if (st === 'stagger') { const w = Math.sin(t * 14) * 0.12; q.lean = -0.22; q.yaw = w; q.armL = { x: -0.4, y: 0, z: -1.4 }; q.armR = { x: -0.3, y: 0, z: 1.1 }; q.foreL = -0.3; q.foreLy = 0.6; q.foreR = -0.3; q.headX = -0.3; q.bodyY = -0.06; q.hipR = -0.2; q.hipL = -0.2; q.kneeR = 0.4; q.kneeL = 0.4; q.cape = 0; q.tumble = 0; q.roll = 0; rate = 18; }
    else if (st === 'dead') {
      const u = clamp(this.deadT / 0.6, 0, 1); const e = ease(u);
      q.tumble = -Math.PI / 2 * e; q.bodyY = -0.72 * e; q.armR = { x: -0.4 * e, y: 0, z: 1.3 * e }; q.armL = { x: -0.3 * e, y: 0, z: -1.3 * e }; q.foreR = -0.2; q.foreL = -0.2; q.foreLy = 0.3; q.headX = -0.3 * e; q.hipR = 0.1; q.hipL = -0.1; q.kneeR = 0.4 * e; q.kneeL = 0.2; q.lean = 0; q.cape = 0.2; q.roll = 0; rate = Infinity;
    }
    else idle();
    if (this.landT > 0 && (st === 'idle' || st === 'run')) { const k = this.landT * 6; q.bodyY = (q.bodyY || 0) - 0.12 * k; q.kneeR = (q.kneeR || 0) + 0.7 * k; q.kneeL = (q.kneeL || 0) + 0.7 * k; q.hipR = (q.hipR || 0) - 0.4 * k; q.hipL = (q.hipL || 0) - 0.4 * k; q.lean = (q.lean || 0) + 0.15 * k; }
    this.rig.blend(q, rate, dt);
  }
  attackPose(q) {
    const a = this.attack, n = a.name, u = a.u, ph = a.phase;
    const base = { foreR: -0.1, foreL: -0.5, foreLy: 0.3, hipR: 0, hipL: 0, kneeR: 0.1, kneeL: 0.1, headX: 0, cape: 0.3, tumble: 0, roll: 0, bodyY: 0, armL: { x: 0.2, y: 0, z: -0.5 } };
    ...
    if (ph === 'windup') { const k = ease(u); q.armR = lerpP({ x: 0.12, y: 0, z: 0.14 }, wind, k); q.yaw = windYaw * k; q.lean = windLean * k; q.foreR = -0.3 * (1 - k); if (n === 'heavy') { q.armL = { x: 1.0, y: 0, z: -0.7 }; q.bodyY = -0.05 * k; } }
    else if (ph === 'active') { const k = Math.min(1, u * 1.35); q.armR = lerpP(wind, strike, k); q.yaw = windYaw + (strikeYaw - windYaw) * k; q.lean = windLean + (strikeLean - windLean) * k; q.bodyY = -0.12 * hipDip * k; q.kneeR = 0.1 + 0.6 * hipDip * k; q.kneeL = 0.1 + 0.9 * hipDip * k; q.hipR = -0.3 * hipDip * k; q.hipL = 0.35 * hipDip * k; if (n === 'heavy') q.armL = { x: 0.6, y: 0, z: -0.8 }; }
    else { const k = ease(u); q.armR = lerpP(strike, { x: 0.12, y: 0, z: 0.14 }, k); q.yaw = strikeYaw * (1 - k); q.lean = strikeLean * (1 - k); q.bodyY = -0.12 * hipDip * (1 - k); q.kneeR = 0.1 + 0.6 * hipDip * (1 - k); q.kneeL = 0.1 + 0.9 * hipDip * (1 - k); q.hipR = -0.3 * hipDip * (1 - k); q.hipL = 0.35 * hipDip * (1 - k); }
    if (n === 'air') { q.hipR = -0.8; q.hipL = 0.3; q.kneeR = 1.2; q.kneeL = 0.5; q.cape = 0.9; }
  }
}
