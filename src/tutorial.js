...
export const STEPS = [
  ...
  { id: 'dash', title: 'Dash', html: `Press ${KB('Shift', 'B')} to dash. Do it once on the ground and once in the air: a jump and a dash together cross the widest gaps.`, done: g => g.player.stats.dashes >= 1 && g.player.stats.airDashes >= 1 },
  { id: 'strike', title: 'Strike', html: `Face the captive goblin and strike with ${KB('LMB', 'X')} (or ${KB('J')}). Three strikes chain into a combo.`, done: g => g.dummy.hits.light >= 3, focus: true },
  ...
  { id: 'guard', title: 'Guard', html: `The goblin will swing at you now. Hold ${KB('RMB', 'LB')} (or ${KB('L')}) facing it to guard. Guarding drains stamina.`, onStart: g => { g.dummy.mode = 'swing'; }, done: g => g.dummy.results.blocked >= 1, focus: true },
  ...
  { id: 'roll', title: 'Dodge', html: `Dodge a swing: dash with ${KB('Shift', 'B')} or roll with ${KB('Ctrl')} as it lands. You cannot be hurt through the middle of either.`, done: g => g.dummy.results.dodged >= 1, focus: true },
